...
- **AI 生成模式优先**：先调 `aigc-video-gen`；仅当其退出码 2（两个平台 env key 都没配）时才走 Stock Footage 模式
- **用户素材优先于 AI 生成**：无论哪种模式，用户提供的素材必须优先使用
- **声画同出**：`aigc-video-gen` 默认开启音频生成，prompt 中要详细描述背景音乐+环境音+对话/旁白
- **无配音模式**：用户明确不需要配音时，`aigc-video-gen` 传 `--no-audio`；Stock Footage 模式跳过 TTS 步骤
