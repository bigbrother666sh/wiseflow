...
脚本必须包含**片段拆分规划**——每个片段对应一次 `aigc-video-gen` 调用或一个用户素材，时长不超过模型限制。
...
读取 `raw_article.md`（追爆报告），按报告中的内容结构、爆款元素和可复用点生成脚本。**不套用三段式结构**，而是按照追爆报告拆解的原视频结构来组织脚本，保留叙事节奏和钩子类型。
...
- **调用 `aigc-video-gen` 时，必须把本段对应的音色描述逐字拼进 `--prompt`**（旁白段拼旁白音色，人物对话段拼对应角色音色），逐字复用、不得改写、换词、增删修饰——这是声画同出下成片声音统一的唯一保证
...
- **每个 AI 生成片段的「音频描述」必须写明三层**（声画同出，aigc-video-gen 照此生成声音，定稿时用户确认的就是成片实际听到的）：
...
##### 2.2.3 slideshow-risk 自检清单（定稿前必跑）
...
#### 2.3.5 Gate 0 — 关键帧 contact sheet 确认（定稿后、生产前）

**脚本定稿后、进入打分和生产前，强制走 Gate 0**——用 `siliconflow-img-gen` 出每段关键帧静图，合成 contact sheet 发用户全段确认，**改文字免费、重生一张图远比重跑一段视频便宜**。这是替代"裸 aigc-video-gen 串行生成 + 逐段肉眼确认"的脆弱闸门——把隐喻/构图错误拦在烧视频 API 费之前。
...
1. **逐段生成关键帧静图**——对片段规划表里每段「画面描述」转成英文 prompt，调 `siliconflow-img-gen`：
   ...
   - **用户参考图段（A.3 r2v）**：用 image-edit 模式 `--image <用户参考图>` 生静图，构图贴近未来 aigc-video-gen 会出的视频
   - ⚠️ 这一步**只生静图不调 aigc-video-gen**——目的是用图片确认构图，不烧视频 API 费
...
3. **批量部分通过**——只有用户标"通过"的段才进 aigc-video-gen 批量队列：
   - 通过段 → 进 Step 4 视频素材生产
   - 要改的段 → 改 prompt 后重生该段关键帧，重新拼 contact sheet 让用户确认（递增 `contact-sheet-v2.jpg`、`v3.jpg`，不覆盖旧版便于对比）
   - 全段通过 → 进 2.4 打分流程

4. **Gate 0 旁路条件**（只在以下情况跳过）：
   - `AWK_API_KEY` 未配（siliconflow-img-gen 不可用）→ 向用户报告"Gate 0 不可用，直奔 aigc-video-gen 逐段生成 + 逐段确认"，等用户决策
...
**脚本规划定稿 + Gate 0 通过 + 2.4 打分通过后，进入生产前强制输出全片预算估算**——用户确认估算再开跑，不准"边跑边发现贵"。这是替代上一轮"裸 aigc-video-gen 串行烧 API 费"的脆弱闸门。
...
1. **API 调用次数**：
   ...
   - 视频生成调用 = 段数（每段一次 `aigc-video-gen`，含候选链 fallback 时的重试预估——每段加 1 次兜底重试预算）
...
- 每段 `aigc-video-gen` 跑完记 `actual.segments[i].wall_s`（实际墙钟秒）+ `actual.segments[i].cost_cny`（按平台价目套实际时长）
- 候选链 fallback 触发时记 `actual.segments[i].fallback_to` + 追加费用（decisions.log 也会落，此处只算钱）
- **超估算 ±20% 时向用户报告**："段 NN 实际耗时/费用超估算 X%，是否继续"——不准擅自继续
- 全片完工后比对 `expected` vs `actual`，落 `budget.variance_pct` 给后续 calibration 喂数——帮 calibrator 校准未来项目的预测准度

**budget.json 与 decisions.log 的分工**：
- `decisions.log`：append-only 决策审计，aigc-video-gen fallback 每次落一条，手不准编辑
