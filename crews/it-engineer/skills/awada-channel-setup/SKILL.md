...
仅当 `node_modules` 被清理、`package.json` 变更、或日志报 `Cannot find module 'ioredis'`（plugin=awada）时，才手动补装：
...
1. `Cannot find module 'ioredis'` → 步骤 1 预装未就位（Docker 镜像 build 漏装 / 源码部署 apply-addons.sh 没跑）；手动 `cd <PROJECT_ROOT>/awada && pnpm install --prod` 补装
