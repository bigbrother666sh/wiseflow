...
#
# 用法：
#   dna-eval.sh --platform wx_mp --check                       # 廉价阈值检查（heartbeat 每日）
#   dna-eval.sh --platform wx_mp                               # 聚合触发 DNA 的证据
#   dna-eval.sh --platform wx_mp --dna-id dna-0 --force        # 手动触发（不达阈值也评估）
#   dna-eval.sh --platform wx_mp --mark-evaluated --ids 3,4,5  # 评估完成后标记
set -euo pipefail
SELF="${BASH_SOURCE[0]}"
