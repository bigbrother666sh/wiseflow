---
name: xianyu-ops
description: 闲鱼（goofish.com）商品搜索、查看详情、私信会话管理与回复。通过 forked camoufox-cli 持久化 session xianyu 完成。
metadata:
  openclaw:
    emoji: 🐟
---
...
wrapper 转发到包内 `scripts/xianyu_search.py`。脚本内部：open 搜索页加载 mtop lib → 逐页 `camoufox-cli eval` 调 `window.lib.mtop.request` → 解析 `data.resultList` → 输出 JSON。
