...
抖音短视频的内容风格提取与 DNA 生产工具。输入是**视频的转录文本**（口播 ASR 转录 + 标题/描述/互动线索，由 Agent 先行整理成 `.md` / `.txt`），不是视频文件本身；视频拆解与转录由 `viral-chaser` 完成，本工具只消费转录文本与封面 / 首帧图。
...
- `--cover-image`：封面或首帧本地图片（`viral-chaser` 抽帧产物或封面下载图），进入视觉模型分析。
