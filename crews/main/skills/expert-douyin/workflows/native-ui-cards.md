...
主题限定自媒体获客。写明常见看法与真正有依据的反常识判断，避免「大家本来就知道」的观点和绝对化口号。真实业务数字和案例必须可核验；聊天与评论要符合自然交流习惯。确认选题、观众及要引发的讨论后继续。
...
界面不加说明性角标；群人数、时间戳和可选 `stats` 按画面语境设置，检查前后一致。群友头像可从打包池选，也可以按许可另下载或 AIGC 生成，在 `avatar_sources` 写来源；不能拿真实客户肖像充当虚构群友。
