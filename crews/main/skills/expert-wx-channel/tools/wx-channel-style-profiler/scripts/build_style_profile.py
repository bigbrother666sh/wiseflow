#!/usr/bin/env python3
"""wx_channel-style-profiler：按作品类型（video / note）生成定性 DNA 资产。
...
KIND_SUFFIX_HINT = "视频号只有视频作品，无需分类型建 DNA"
...
DIMENSION_GROUPS = {
    "video": {
        "选题与包装": [("topic-angle", "选题与观看理由"), ("title-cover", "短标题、视频描述与封面")],
...
REPORT_DIMENSION_PROMPTS = {
    ...
    "title-cover": "- 单篇观测：**短标题**（发布页可填，官方称填了能获得更多流量；作品管理页不展示，取数时拿不到）与**视频描述**（含话题标签）两者的原文与写法模式。\n- 取数边界：抓取到的作品只有视频描述；短标题拿不到时写「未观测（管理页不展示）」，不得把描述当短标题。\n- 视觉证据：封面或首帧必须由视觉模型读取图片，至少提取画面主体与场景、构图与画幅、色彩体系、光线与质感、风格与媒介、文字视觉、品牌识别元素、避免项，并反推为可执行的 AIGC 提示词要素；无图片写「未提供」。",
    "content-idea": "- 单篇观测：一句话创意内核、创意类型、展开逻辑（悬念 / 反转 / 递进 / 对比 / 清单 / 实测）、记忆点。\n- 可复用信号：这个创意套路换成别的主题还能怎么用。\n- 边界：只记创意层，不记创作细节（逐句台词、镜头表、脚本结构、转场与编码参数）。",
    "video-form": "- 单篇观测：视频内容形态（口播 / 实拍拼接 / 影视解说+反转植入 / 纯 AIGC 动画 / 创意转场动效 / 录屏演示 / 图文卡片视频 / 混合）与判定依据（画面证据、口播占比、素材来源）。\n- 制作指向：必须落到真实存在的资源名——Content Producer `expert-video` 的某个 workflow（Reversal Ad / Deck Talk / Collage B-roll；不属这三类就写「不指定类型 workflow」，由 CP 按通用制作流程据创意自定手法），或 main 的素材加工技能（`video-edit` / `talking-head-cut` / `ui-demo`）；不得发明不存在的名字。",
...
TEMPLATE_STAGES = {
    "video": ("选题", "短标题与视频描述", "内容创意", "业务植入与 CTA", "视频形态与制作指向", "制作规格", "口播文案"),
}

TEMPLATE_STAGE_FIELDS = {
    "选题": ("选题角度推荐", "选题需考虑的受众关联角度", "内容支柱与系列关系", "避免"),
    "短标题与视频描述": ("短标题模式", "参考短标题", "视频描述结构", "参考视频描述", "话题标签策略", "封面风格与 AIGC 要素"),
