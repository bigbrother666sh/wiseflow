...
REPORT_DIMENSION_PROMPTS = {
    ...
    'search-intent-map': '- 搜索证据：主关键词、相关词、长尾句和用户可能提问。\n- 聚合边界：单篇只给候选；账号级搜索意图地图需多样本聚合。',
...
def dna_document_markdown(
    ...
    return "\n\n".join(
        [
            ...
            "---",
            f"# {dna_id} DNA 文档",
            "本文件聚合历史 DNA report。它是账号当前采用的账号级运营规则，也必须能推导出 main agent 的图文生产输入或视频制作 Brief。",
            "## 报告与权重",
            "\n".join(
...
TEMPLATE_STAGES = ("定位与核心传达", "选题与标题包装", "内容形式与发布节奏", "高数据创意模式", "互动与系列", "搜索意图与用户问题", "制作交接与Pipeline", "口播文案DNA")
...
def report_command(args: argparse.Namespace) -> None:
    ...
    output_dir = Path(args.output_dir or f"xhs/dna/{args.dna_id}/reports")
    cover_image = ""
    if args.cover_image:
...
def build_command(args: argparse.Namespace) -> None:
    validate_id(args.dna_id, "--dna-id")
    input_values = args.input or [f"xhs/dna/{args.dna_id}/reports"]
    ...
    output_dir = Path(args.output_dir or f"xhs/dna/{args.dna_id}")
