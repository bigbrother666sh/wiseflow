...
- 选题只围绕自媒体获客，反差要给读者一个可验证的新判断。观点保留条件，不写无依据的绝对断言。案例数字、客户结果和真实互动数据必须可核验；不能把示例或演绎中的数字当成真实业绩。
...
- 模拟聊天和评论界面保留「情景演绎」标识。`verified_stats` 只填真实且核验过的界面数据；没有证据就留空。发布文案也不得暗示这些演绎对话是真实群聊或真实平台评论。
