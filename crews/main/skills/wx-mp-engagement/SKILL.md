...
# 失效后：camoufox 扫码登录
/Users/wukong/projects/wiseflow/crews/main/skills/wx-mp-hunter/scripts/wx-mp-hunter.sh login           # camoufox 无头截 QR PNG 落 /tmp/qr-wx-mp.png
# （发 QR PNG 给用户 -> 用户扫码后 -> 主会话回复"已扫码"）
/Users/wukong/projects/wiseflow/crews/main/skills/wx-mp-hunter/scripts/wx-mp-hunter.sh login-confirm   # 验登录就位 + 导出 cookie+UA+token 落中央存储
```
...
/Users/wukong/projects/wiseflow/crews/main/skills/wx-mp-engagement/scripts/wx-mp-engagement.sh probe
# 产物落在 ./wx-mp-engagement-probe/：01_center.png / 02_list.png / 02_list.html / 03_articles.json

# 列出后台所有文章 + 行内 metrics
/Users/wukong/projects/wiseflow/crews/main/skills/wx-mp-engagement/scripts/wx-mp-engagement.sh list

# 抓单篇（按 row.title 在列表页匹配）
/Users/wukong/projects/wiseflow/crews/main/skills/wx-mp-engagement/scripts/wx-mp-engagement.sh fetch --row-id <pub_wx_mp.id>

# 批量抓取最近 N 天未更新（reads=0）的所有 wx_mp 记录
/Users/wukong/projects/wiseflow/crews/main/skills/wx-mp-engagement/scripts/wx-mp-engagement.sh fetch-all --days 7
```
...
```
1. /Users/wukong/projects/wiseflow/crews/main/skills/wx-mp-hunter/scripts/wx-mp-hunter.sh check
...
#   2. wx-mp-engagement.sh fetch --row-id 42  ← 本 skill（crew 专属 wrapper，由 fetch-and-update-metrics 内部按部署路径调，agent 不裸调）
...
- **症状**：列表页显示"请重新登录"
- **workaround**：token 与 wx_mp session 同寿命，失效则 `/Users/wukong/projects/wiseflow/crews/main/skills/wx-mp-hunter/scripts/wx-mp-hunter.sh check` exit 2 → 走 wx-mp-hunter `login` + `login-confirm` 重登流（重登后 token 随 cookie+UA 一并重新导出落中央存储），再用新 token 拼列表页 URL
