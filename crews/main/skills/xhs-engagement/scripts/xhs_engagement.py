...
def update_metrics_row(row_id: int, metrics: dict) -> dict:
    """写库委托 published-track 的 update-metrics.sh（pub_xhs 列：views/likes/favorites/comments/shares）"""
...
def stats_to_metrics(stats: list[int]) -> dict:
    """creator 后台 stats 区 5 列数字顺序：阅读/点赞/收藏/评论/分享"""
    padded = (stats + [0] * 5)[:5]
    return {
        "views": padded[0],
        "likes": padded[1],
