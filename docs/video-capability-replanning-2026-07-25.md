...
| 2026-07-25 | **技能内容规范第 6 节落地**：SKILL.md 与 crew 专属 scripts 禁写开发判断/产品取舍/参考来源词（借鉴 OpenMontage/borrowed from MPT/范式配套/有意的边界 等），只能写 docs/。SKILL.md 删两处 + assemble.py L259 删"借鉴 OpenMontage" + review.py L8/9/68/69/78 删"borrowed from OpenMontage + gbro Gate 3 QA"等五处 + Usage 节路径订正；调研文档补第 6 节 5 子节（规范正文/判据/落地位置/自检清单/已清理记录） |
| 2026-07-25 | commit `f5c99a1` 推 origin/fix/it-engineer-channel-layering；远端合并后删分支，本地 master 快进同步远端 a002109 + 删本地 fix 分支 |
| 2026-07-25 | **highlight-cut 独立技能落地**（todo H 3.11.4 修正项）：新建 `crews/main/skills/highlight-cut/` 独立技能——①SKILL.md 5 步工作流（检测生 cut_plan.json → 用户确认 plan → 按 plan 剪拼 → 成片自检强制闸门 → 用户确认）+ reason 枚举表 + 依赖清单（ffmpeg/ffprobe + 火山 ASR env + requests）+ 禁止事项（禁跳 review.py 交付 / 禁硬剪不确认 plan / 禁直接写 ffmpeg 命令）；②`scripts/cut_plan.py`（541 行）——火山方舟豆包语音极速版 ASR 拿 utterance + word 级时间戳 + 多层检测（fillers/silence/stutters/false_starts/repeats/highlight）+ 三 mode（filler/highlight/both）+ Jaccard 相似度判据 + stdout 报告（按 reason 分组统计 keep/remove 段数与时长）；③`scripts/apply_cut.py`（267 行）——按 cut_plan.json 滤 keep=true 段 + ffmpeg 逐段抽（libx264 crf 20 + aac 128k + scale/pad 保持分辨率 + 段间 triangular fade 防咔点）+ concat 拼接 + ffprobe 探源分辨率帧率。与 video-assembler 完全脱钩，不胉 video-assembler/scripts/。火山 ASR 鉴权复用 viral-chaser 约定（VOLC_ASR_APP_ID+VOLC_ASR_ACCESS_KEY 旧控制台双头 或 VOLC_ASR_APP_KEY 新控制台单头）。本技能满足用户"参考 AutoClip 为 Main Agent 增加高光剪辑 Skill"需求，落地方式按调研原则：只吸收算法/策略/工作流，不抄 Provider 适配层、不引入 UI 层。 |
| 2026-07-25 | **MPT 脚本定义追研**（todo H 之外）：MPT "AI 自动生成视频脚本"的"脚本"是**旁白文案脚本**（narration script）——`generate_script` 产纯文本给 TTS 朗读，不是字幕脚本（字幕走 faster-whisper ASR 另条链），也不是镜头脚本（MPT 不分镜，时长靠 video_count/audio_duration 参数控）。`generate_terms` 吃脚本出素材搜索关键词，是两阶段。**与 main 完全脱钩**——main 不出脚本不烧字幕，"脚本"二字不进 video-assembler；归 content-producer 范畴（出发点 1.1）。3.4 节"出脚本（LLM）"行表述确认无误 |
| 2026-07-25 | **MPT moviepy 合成方案追研**：底层就是 ffmpeg（moviepy 2.x 是 ffmpeg 的 Python wrapper，L161-167/L423-441 明示），与我们的范式本质同源。moviepy 提供的转场能力清单（fade_in/fade_out/slide_in/slide_out/zoom_in/zoom_out/shuffle/none 七种，L667-687）与 ffmpeg `xfade` filter 一一对应——**不引入 moviepy 也能拿到同等能力**。调研结论：转场清单思路可借鉴，但走 ffmpeg xfade filter 不引 moviepy；后续 assemble.py 若扩展 `--transition fade/slide/zoom/shuffle` 走 xfade 即可。3.6/3.7 节"不引入 moviepy"判定不变，但理据从"重依赖"补强为"moviepy 只是 ffmpeg wrapper，xfade filter 一一对应无需引入" |
| 2026-07-25 | **CP video-product skill 重构落地**：①SKILL.md 从 1 行占位重写为 234 行完整工作流文——出脚本→Gate 0 关键帧确认→预算估算→逐段 AI 生成/素材组装→合成→成片自检→封面→用户确认全链路，作三条平行路径（collage-broll/html-video/manim-explainer）的编排入口；stages/ 6 子文档保留不动，SKILL.md 只做主骨架+导航指针+工作流串联；②删 4 冗余脚本：`gen.py`+`tts.py`（已抽公共 `aigc-video-gen`/`siliconflow-tts`，CP 副本删走 PATH 调用）、`state.py`（同 main 范式用不上）、`extract_and_concat.py`（stages 全无引用，上轮从 main 焯来的冗余）；剩 5 脚本（assemble/check/compress_preview/review）stages 全有引用保留；③stages 里 `gen.py`→`aigc-video-gen`、`tts.py`→`siliconflow-tts` 全替换（36+2 处），4 处不通顺 `python3 ./skills/.../scripts/<skill>` 命令订正为 PATH 调用 `aigc-video-gen`/`siliconflow-tts`；④stages 残留参考来源词清净：`borrowed from OpenMontage storyboard gate`/`借鉴 gbro Gate 2 + OpenMontage storyboard`/`gbro README 原话`/`borrowed from OpenMontage budget gate`/`借鉴 OpenMontage post-render self-review + gbro Gate 3 QA`/`OpenMontage 没有，是我们加的`/`可借鉴点`→`可复用点` 等共 7 处删改，符合第 6 节技能内容规范；⑤frontmatter `name: video-product` + description 三条平行路径编排入口 + 工作区结构含 keyframes/calibration/budget.json/raw_article.md/script.md/character_reference.jpg；⑥脚本清单表 6 行——aigc-video-gen/compress_preview/assemble/review/check/siliconflow-tts；⑦禁止事项加"禁止跳过 review.py 交付"第 3 条。校验：4 脚本字节码编译全过 + SKILL.md+stages grep 残留扫净（"Gate 0"是工作流文留）+ git 状态符合预期（SKILL.md M + 4 脚本 D + 7 stages M） |
