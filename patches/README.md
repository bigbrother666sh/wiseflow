...
### 3. Patchright 版本（overrides.sh 默认）

`overrides.sh` 默认 `PATCHRIGHT_VERSION=1.61.1`（2026-07-05 升自 1.60.2，跟 Playwright 1.61 同步）。可通过环境变量覆盖：

```bash
PATCHRIGHT_VERSION=1.60.2 bash overrides.sh  # 锁定旧版
```
